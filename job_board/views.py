...
from .permissions import (
    IsAdminUser,
    IsApplicantOrAdmin,
    IsApplicantOrAdminUser,
    IsOwnerOrAdmin,
    IsParticipantOrAdmin,
    IsRecruiterOrAdminUser,
)
...
class CompanyViewSets(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        ...
        #     lambda: send_company_registration_confirmation_email.delay(
        #         company.email,
        #         company.name,
        #         company.location,
        #         company.industry
...
class CompanyJobApplicationsViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if role == "recruiter" and job.company.user != self.request.user:
            raise PermissionDenied(
                "You cannot access applications for another company."
            )
    ...
    def perform_create(self, serializer):
        ...
        if not participants:
            raise ValidationError(
                "At least one participant (receiver) must be provided "
...
class MessageViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        ...
        if (
            parent_message
            and parent_message.conversation_id != conversation.conversation_id
        ):
            raise ValidationError("Reply message must belong to the same conversation.")

        receiver = (
            conversation.participants.exclude(user_id=self.request.user.user_id)
            .order_by("user_id")
            .first()
    ...
    def _get_recursive_replies(self, parent_message):
        replies = list(
            Message.objects.filter(parent_message=parent_message)
